...
// use for of loop to iterate through object
for (let x of watchList) {
    // log(x.imdbRating, typeof (x.imdbRating));
    // log(typeof (Number(x.imdbRating)));

    // change rating type to number
    const ratings = Number(x.imdbRating);
    const marker = 8.0;
    log(typeof (ratings), marker);

    const y = ratings >= marker ? log(`Title: ${x.Title}, Ratings: ${x.imdbRating}`) : null;
}
