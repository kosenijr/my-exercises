...
function add(oldList, bookName) {
    let newList = [];
    newList.push(...oldList, bookName); // use push method and spread operator to make new and old lists completely distinct: comma breaks sequence of push action: first original list then 
    // newList.push(bookName); // include last element, which is the addition
    //   bookList.push(bookName);
    console.table(newList); // new list takes the onus of modification from old list
    console.table(oldList); // impervious to changes taking place
    return bookList;
