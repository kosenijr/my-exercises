...
// let's filter
const mapped = watchList.map(film => `Title: ${film.Title}, Ratings: ${Number(film.imdbRating)}`);
const filtered = watchList.filter(film => Number(film.imdbRating) >= 8.0 ? mapped : null);
log(filtered);

log(mapped);
// ternary condition
filteredList = filtered ? mapped : null;
