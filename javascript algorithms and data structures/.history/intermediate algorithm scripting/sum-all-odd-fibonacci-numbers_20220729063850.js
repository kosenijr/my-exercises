...
const sumFibs = (num) => {
    ...
    // second, use reduce method to create next step, then push
    arr.map(elem => elem === arr.at(-1) && elem <= num ? arr.push(arr.reduce((prevVal, currVal) => prevVal + currVal, 0)) : null);
    // use map to continue with last step
    // arr.map(elem => elem <= num && elem === arr.at(-1) ? arr.push(elem + arr.at(-2)) : null);
    //
    // check arr
    log(arr)
    // 
